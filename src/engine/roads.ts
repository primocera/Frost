...
const LANES: { x: number; y: number }[][] = [
  ...
  // Left fork: out → down a bit → continue left to the forest's west edge.
  [{ x: STARTER_X, y: SPLIT_Y }, { x: 2780, y: SPLIT_Y }, { x: 2780, y: SPLIT_Y + 260 }, { x: 1560, y: SPLIT_Y + 260 }],
  // Right fork: out → down a bit → continue right to the forest's east edge.
  [{ x: STARTER_X, y: SPLIT_Y }, { x: 3820, y: SPLIT_Y }, { x: 3820, y: SPLIT_Y + 260 }, { x: 5060, y: SPLIT_Y + 260 }],
]
const LANE_W = 80
